...
void Model::MakeMaterial(const void* ptr, std::string directory)
{
	...
	for (unsigned int i = 0; i < m_materials.size(); ++i)
	{
		...
		// モデルと同じ階層の texture サブフォルダを探索
		hr = m_materials[i].pTexture->Create((directory + "texture\\" + fileName).c_str());
		if (SUCCEEDED(hr)) { continue; }

		// モデルと同じ階層の textures サブフォルダを探索
		hr = m_materials[i].pTexture->Create((directory + "textures\\" + fileName).c_str());
		if (SUCCEEDED(hr)) { continue; }
