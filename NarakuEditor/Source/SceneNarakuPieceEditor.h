...
public:
    ...
    enum NativeMenuCommand : unsigned int
    {
        ...
        /** @brief 現在のピースを削除します。 */
        MenuDeletePiece,
        /** @brief 保存済みの小ステージをゲーム側へ発行します。 */
        MenuPublishPieces,
        /** @brief 直近の小ステージ発行バックアップを復元します。 */
        MenuRestorePublishedPieces,
        ...
        /** @brief 3Dプレビューの地形テクスチャ表示を切り替えます。 */
        MenuToggleGroundTextures
    };
    ...
    /**
     * @brief Generated Previewへ移動する前に未保存の編集内容を確認します。
     * @return 移動を続行できる場合はtrue、キャンセルされた場合はfalseを返します。
     */
    bool PrepareToOpenGeneratedPreview();
...
private:
    ...
    /** @brief 小ステージ中央へ表示する拠点モデル資源です。 */
    struct BasePreviewModelResource
    {
        Model* model = nullptr;
    ...
    /** @brief 水場の半透明面を構成する1頂点です。 */
    struct WaterOverlayVertex
    {
    ...
    /** @brief 通常経路から到達できない歩行可能領域の検出結果です。 */
    struct UnreachableWalkableResult
    {
    ...
    /** @brief 到達不能な歩行可能領域を検出した時の保存確認を描画します。 */
    void DrawUnreachableWalkablePopup();
     ...
     */
    bool SavePiece(bool saveAsDraft, bool allowUnreachableWalkable = false);

    /** @brief 接続可能な外周や進入地点から到達できない歩行可能領域を検出します。 */
    UnreachableWalkableResult FindUnreachableWalkableRegions() const;

    /** @brief 検出した到達不能セルを地形セル選択へ反映します。 */
    void SelectUnreachableWalkableCells();
    ...
    void CreateNewPiece(
        ...
        NarakuPiece::SizePreset sizePreset = NarakuPiece::SizePreset::Size16x16);
    ...
    /** @brief 前衛拠点と第二拠点のプレビュー用モデルを読み込みます。 */
    void LoadBasePreviewModels();

    /** @brief 拠点プレビュー用モデルを解放します。 */
    void ReleaseBasePreviewModels();

    /** @brief 地上施設の直接指定モデル用プレビューキャッシュを解放します。 */
    void ReleaseSurfaceFacilityPreviewModel() const;

    /** @brief 選択中の拠点種別に対応するモデルを中央へ描画します。 */
    void DrawBasePreview3D() const;

    /** @brief 水場セルを面で表示する描画資源を初期化します。 */
    void InitializeWaterOverlayPreview();

    /** @brief 水場セルの面描画資源を解放します。 */
    void ReleaseWaterOverlayPreview();

    /** @brief 水場セルへ地形形状に沿った半透明水色面を描画します。 */
    void DrawWaterOverlays3D() const;

    /** @brief 地形セルを設定されたテクスチャ付きで描画します。 */
    void DrawGroundTextures3D() const;

    /** @brief 登坂上限と対象セルの傾斜角を3Dプレビュー上に表示します。 */
    void DrawSlopeInfoOverlay() const;
    ...
    /** @brief 選択セルの水場深度を一括更新します。 */
    void ApplySelectedCellWaterDepth(NarakuPiece::WaterDepth waterDepth);
    ...
    int FindFishingPointIndexByCell(int cellX, int cellZ) const;
    bool IsFishingPointWaterCell(int cellX, int cellZ) const;
    bool CanChangeFishingCellWaterDepth(
        int cellX,
        int cellZ,
    ...
    std::string GenerateFishingPointId() const;
    ...
    void SelectFishingPoint(int index);
...
private:
    ...
    /** @brief 水場属性ツールでセルへ設定する水場の種類です。 */
    NarakuPiece::WaterDepth m_newWaterDepth = NarakuPiece::WaterDepth::Puddle;
    ...
    int m_selectedFishingPointIndex = -1;

    /** @brief Assetsウィンドウへ登録済みの環境モデル一覧です。 */
    std::vector<EnvironmentModelAsset> m_environmentModels;

    /** @brief 第二拠点、前衛拠点の順に保持するプレビュー用モデルです。 */
    std::array<BasePreviewModelResource, 2> m_basePreviewModels = {};
    mutable Model* m_surfaceFacilityPreviewModel = nullptr;
    mutable std::string m_surfaceFacilityPreviewPath;
    mutable DirectX::XMFLOAT3 m_surfaceFacilityPreviewAnchor = {};
    mutable unsigned long long m_surfaceFacilityPreviewWriteTime = 0;

    static constexpr std::size_t WaterOverlayVertexCapacity = 15u * 15u * 6u;
    /** @brief 16x16小ステージ内の水場面をまとめて描画する動的バッファです。 */
    MeshBuffer* m_waterOverlayMesh = nullptr;
    VertexShader* m_waterOverlayVS = nullptr;
    PixelShader* m_waterOverlayPS = nullptr;
    Texture* m_waterOverlayTexture = nullptr;
    mutable std::array<WaterOverlayVertex, WaterOverlayVertexCapacity> m_waterOverlayVertices = {};

    static constexpr std::size_t GroundTextureCount = 4u;
    /** @brief 草地、土、岩、湿地の地形プレビュー用テクスチャです。 */
    std::array<Texture*, GroundTextureCount> m_groundTextures = {};
    ...
    /** @brief 新規作成する小ステージが地上用ならtrueです。 */
    bool m_newPieceIsSurface = false;
    ...
    /** @brief 次回描画で到達不能領域の保存確認を開くための要求フラグです。 */
    bool m_requestOpenUnreachableWalkablePopup = false;

    /** @brief 到達不能領域の保存確認で選択中の処理です。0=ロープ、1=地形、2=そのまま保存です。 */
    int m_unreachableWalkableAction = 1;

    /** @brief 到達不能領域の確認後に保存する形式です。 */
    bool m_pendingUnreachableSaveAsDraft = true;

    /** @brief 保存確認の対象になった到達不能セルです。 */
    std::vector<CellSelection> m_unreachableWalkableCells;

    /** @brief 保存確認の対象になった到達不能領域数です。 */
    int m_unreachableWalkableRegionCount = 0;
    ...
    /** @brief 3Dプレビューで地形テクスチャを表示するかどうかです。 */
    bool m_showGroundTextures = true;
