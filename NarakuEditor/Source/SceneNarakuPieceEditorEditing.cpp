...
XMFLOAT3 SceneNarakuPieceEditor::GetVertexWorldPosition(int x, int z) const
{
    ...
    return
    {
        offsetX - static_cast<float>(x) * m_piece.cellSize,
        GetHeight(x, z),
...
int SceneNarakuPieceEditor::FindFishingPointIndexByCell(int cellX, int cellZ) const
{
    for (size_t index = 0; index < m_piece.fishingPoints.size(); ++index)
        if (m_piece.fishingPoints[index].shoreCell.x == cellX && m_piece.fishingPoints[index].shoreCell.z == cellZ)
            return static_cast<int>(index);
    return -1;
}

bool SceneNarakuPieceEditor::IsFishingPointWaterCell(int cellX, int cellZ) const
{
    return std::any_of(m_piece.fishingPoints.begin(), m_piece.fishingPoints.end(),
        [cellX, cellZ](const NarakuPiece::FishingPointData& point)
        {
            return point.waterCell.x == cellX && point.waterCell.z == cellZ;
        });
}

bool SceneNarakuPieceEditor::CanChangeFishingCellWaterDepth(
    int cellX,
    int cellZ,
    NarakuPiece::WaterDepth waterDepth,
    std::string& outMessage) const
{
    if (FindFishingPointIndexByCell(cellX, cellZ) >= 0 &&
        waterDepth != NarakuPiece::WaterDepth::None &&
        waterDepth != NarakuPiece::WaterDepth::Puddle)
    {
        outMessage = u8"釣り地点の岸は陸地または溜りである必要があるため、水場属性を変更できません";
        return false;
    }
    if (IsFishingPointWaterCell(cellX, cellZ) &&
        waterDepth != NarakuPiece::WaterDepth::Pond &&
        waterDepth != NarakuPiece::WaterDepth::Lake)
    {
        outMessage = u8"釣り地点の対象水面は池または湖である必要があるため、水場属性を変更できません";
        return false;
    }
    return true;
}
...
std::string SceneNarakuPieceEditor::GenerateFishingPointId() const
{
    int maxNumber = 0;
    for (const NarakuPiece::FishingPointData& point : m_piece.fishingPoints)
    {
        int number = 0;
        if (sscanf_s(point.id.c_str(), "fishing_%d", &number) == 1) maxNumber = std::max(maxNumber, number);
    }
    char buffer[32] = {};
    std::snprintf(buffer, sizeof(buffer), "fishing_%02d", maxNumber + 1);
    return buffer;
}
...
void SceneNarakuPieceEditor::SelectFishingPoint(int index)
{
    if (index < 0 || index >= static_cast<int>(m_piece.fishingPoints.size()))
    {
        ClearGridObjectSelection();
        return;
    }
    m_selectedGridObjectKind = GridObjectKind::FishingPoint;
    m_selectedFishingPointIndex = index;
    m_selectedMiningPointIndex = -1;
}
...
bool SceneNarakuPieceEditor::CanPlaceGridObject(GridObjectTool tool, int cellX, int cellZ, std::string& outMessage) const
{
    ...
    if (m_piece.isSurface && m_piece.surfaceFacility.type != NarakuPiece::SurfaceFacilityType::None &&
        cellX >= m_piece.surfaceFacility.cell.x && cellX < m_piece.surfaceFacility.cell.x + 2 &&
        cellZ >= m_piece.surfaceFacility.cell.z && cellZ < m_piece.surfaceFacility.cell.z + 2)
    {
        outMessage = u8"地上施設の2x2占有セルには配置できません";
        return false;
    }

    const bool fishingFromPuddle = tool == GridObjectTool::FishingPoint &&
        cellData->waterDepth == NarakuPiece::WaterDepth::Puddle;
    if (tool != GridObjectTool::Water && !fishingFromPuddle &&
        cellData->waterDepth != NarakuPiece::WaterDepth::None)
    {
        outMessage = u8"水場セルには配置できません";
        return false;
    }

    if (tool == GridObjectTool::Water &&
        !CanChangeFishingCellWaterDepth(cellX, cellZ, m_newWaterDepth, outMessage))
    {
        return false;
    }

    if (tool == GridObjectTool::Water && m_newWaterDepth != NarakuPiece::WaterDepth::None)
    {
        ...
        if (hasMiningPoint || hasRopePoint || hasStartReturn || hasLayerRopePoint ||
            hasLayerLoadPoint || HasEnvironmentObjectAt(cellX, cellZ))
        {
            outMessage = u8"配置物があるセルには水場属性を設定できません";
            return false;
        }
    }
    ...
    const bool cellAttributeTool = tool == GridObjectTool::Water;
    // 条件に該当する場合は、`outMessage` の状態を更新します。
    if (!ropeTool && !cellAttributeTool && HasEnvironmentObjectAt(cellX, cellZ))
    {
        outMessage = u8"環境オブジェクトと同じセルにはロープ以外を配置できません";
        return false;
    }

    if (tool != GridObjectTool::FishingPoint && tool != GridObjectTool::Water &&
        FindFishingPointIndexByCell(cellX, cellZ) >= 0)
    {
        outMessage = u8"釣り地点の岸セルには別の配置物を配置できません";
        return false;
    }
    ...
    case GridObjectTool::MiningPoint:
        if (FindFishingPointIndexByCell(cellX, cellZ) >= 0)
        {
            outMessage = u8"釣り地点と同じセルには配置できません";
            return false;
        }
        // 条件に該当する場合は、`outMessage` の状態を更新します。
    ...
    case GridObjectTool::FishingPoint:
    {
        if (FindMiningPointIndexByCell(cellX, cellZ) >= 0)
        {
            outMessage = u8"採掘ポイントと同じセルには配置できません";
            return false;
        }
        if (!cellData->walkable)
        {
            outMessage = u8"釣り地点は歩行可能な岸セルへ配置してください";
            return false;
        }
        ...
        for (int i = 0; i < 4; ++i)
        {
            const NarakuPiece::CellData* water = GetCellData(cellX + dx[i], cellZ + dz[i]);
            if (water && (water->waterDepth == NarakuPiece::WaterDepth::Pond ||
                water->waterDepth == NarakuPiece::WaterDepth::Lake)) return true;
        }
        outMessage = u8"隣接する池または湖がありません";
        return false;
    }

    case GridObjectTool::Water:
        return true;
...
bool SceneNarakuPieceEditor::DeleteSelectedGridObject()
{
    ...
    case GridObjectKind::FishingPoint:
        if (m_selectedFishingPointIndex < 0 || m_selectedFishingPointIndex >= static_cast<int>(m_piece.fishingPoints.size()))
            return false;
        PushUndoSnapshot();
        m_piece.fishingPoints.erase(m_piece.fishingPoints.begin() + m_selectedFishingPointIndex);
        ClearGridObjectSelection();
        MarkPieceDirty();
        SetMessage(u8"釣り地点を削除しました");
        return true;
...
{
    EDITOR_PROFILE_FUNCTION();
    const bool deletingCell = field == &NarakuPiece::CellData::deleted && value;
    const bool disablingWalkable = field == &NarakuPiece::CellData::walkable && !value;
    if (deletingCell || disablingWalkable)
    {
        for (const CellSelection& selection : m_selectedCells)
        {
            if (FindFishingPointIndexByCell(selection.x, selection.z) >= 0)
            {
                SetMessage(deletingCell
                    ? u8"釣り地点の岸セルは削除できません"
                    : u8"釣り地点の岸セルは歩行不可に変更できません");
                return;
            }
            if (deletingCell && IsFishingPointWaterCell(selection.x, selection.z))
            {
...
void SceneNarakuPieceEditor::ApplySelectedCellWaterDepth(NarakuPiece::WaterDepth waterDepth)
{
    EDITOR_PROFILE_FUNCTION();
    for (const CellSelection& selection : m_selectedCells)
    {
        std::string changeError;
        if (!CanChangeFishingCellWaterDepth(selection.x, selection.z, waterDepth, changeError))
        {
            SetMessage(changeError);
            return;
        }
    }
    PushUndoSnapshot();
...
SceneNarakuPieceEditor::EditorSnapshot SceneNarakuPieceEditor::CreateEditorSnapshot() const
{
    ...
    snapshot.selectedFishingPointIndex = m_selectedFishingPointIndex;
...
void SceneNarakuPieceEditor::RestoreEditorSnapshot(const EditorSnapshot& snapshot)
{
    ...
    m_selectedFishingPointIndex = snapshot.selectedFishingPointIndex;
    ...
    if (m_selectedGridObjectKind == GridObjectKind::FishingPoint &&
        (m_selectedFishingPointIndex < 0 || m_selectedFishingPointIndex >= static_cast<int>(m_piece.fishingPoints.size())))
        ClearGridObjectSelection();
    // 条件に該当する場合は、対応する編集処理を実行します。
...
void SceneNarakuPieceEditor::UpdateGridObjectEditing()
{
    ...
    NarakuPiece::GridPoint clickedFishingWaterCell = {};
    bool fishingWaterClicked = false;
    if (m_gridObjectTool == GridObjectTool::FishingPoint)
    {
        const NarakuPiece::CellData* clickedCell = GetCellData(cellX, cellZ);
        fishingWaterClicked = clickedCell &&
            (clickedCell->waterDepth == NarakuPiece::WaterDepth::Pond ||
                clickedCell->waterDepth == NarakuPiece::WaterDepth::Lake);
        if (fishingWaterClicked)
        {
            clickedFishingWaterCell = { cellX, cellZ };
            static const int shoreDx[] = { 0, 1, 0, -1 };
            static const int shoreDz[] = { -1, 0, 1, 0 };
            bool foundShore = false;
            for (int direction = 0; direction < 4; ++direction)
            {
                const int shoreX = cellX + shoreDx[direction];
                const int shoreZ = cellZ + shoreDz[direction];
                if (FindFishingPointIndexByCell(shoreX, shoreZ) >= 0)
                {
                    continue;
                }

                std::string shoreError;
                if (CanPlaceGridObject(GridObjectTool::FishingPoint, shoreX, shoreZ, shoreError))
                {
                    cellX = shoreX;
                    cellZ = shoreZ;
                    foundShore = true;
            ...
            if (!foundShore)
            {
                SetMessage(u8"水場に隣接する配置可能な未使用の岸セルがありません");
                return;
            }
        }
    }
    ...
    case GridObjectTool::FishingPoint:
    {
        const int existingIndex = FindFishingPointIndexByCell(cellX, cellZ);
        if (existingIndex >= 0)
        {
            SelectFishingPoint(existingIndex);
            SetMessage(u8"既存の釣り地点を選択しました");
            return;
        }
        static const int waterDx[] = { 0, 1, 0, -1 };
        static const int waterDz[] = { -1, 0, 1, 0 };
        NarakuPiece::GridPoint waterCell = clickedFishingWaterCell;
        if (!fishingWaterClicked)
        {
            for (int direction = 0; direction < 4; ++direction)
            {
                const NarakuPiece::CellData* water = GetCellData(
                    cellX + waterDx[direction], cellZ + waterDz[direction]);
                if (water && (water->waterDepth == NarakuPiece::WaterDepth::Pond ||
                    water->waterDepth == NarakuPiece::WaterDepth::Lake))
                {
                    waterCell = { cellX + waterDx[direction], cellZ + waterDz[direction] };
                    break;
                }
            }
        }
        PushUndoSnapshot();
        NarakuPiece::FishingPointData point = {};
        point.id = GenerateFishingPointId();
        point.shoreCell = { cellX, cellZ };
        point.waterCell = waterCell;
        m_piece.fishingPoints.push_back(point);
        SelectFishingPoint(static_cast<int>(m_piece.fishingPoints.size()) - 1);
        MarkPieceDirty();
        SetMessage(u8"釣り地点を追加しました");
        return;
    }
    ...
    case GridObjectTool::Water:
    {
        NarakuPiece::CellData* const cell = GetCellData(cellX, cellZ);
        if (cell == nullptr || cell->waterDepth == m_newWaterDepth)
        {
            return;
        }
        PushUndoSnapshot();
        cell->waterDepth = m_newWaterDepth;
        ClearGridObjectSelection();
        MarkPieceDirty();
        SetMessage(m_newWaterDepth == NarakuPiece::WaterDepth::None
            ? u8"水場属性を解除しました" : u8"水場属性を設定しました");
        return;
    }
