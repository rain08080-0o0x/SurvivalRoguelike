...
void SceneNarakuEditor::ShowGeneratedPreviewWindow()
{
    m_showGeneratedPreviewWindow = true;
}
...
void SceneNarakuEditor::DrawGeneratedPreviewWindow()
{
    ...
    ImGui::Checkbox(u8"開始・帰還地点のあるエリアでスポーン", &m_generatedPreviewSpawnAtReturnArea);
    if (m_generatedPreviewSpawnAtReturnArea)
    {
        ImGui::TextDisabled(u8"開始深度・段階・エリア番号の指定より優先されます。");
    }
    ...
    if (ImGui::Button(u8"15段階を再生成して歩行確認", ImVec2(250.0f, 0.0f)))
    {
        ...
        else
        {
            ...
            config << "spawnAtReturnArea=" << (m_generatedPreviewSpawnAtReturnArea ? 1 : 0) << '\n';
            for (int stage = 0; stage < 15; ++stage)
            {
