...
void SceneNarakuPieceEditor::DrawPieceBasicWindow()
{
    ...
    ImGui::Text("%s %s", u8"小ステージ種別:", m_piece.isSurface ? u8"地上 8x8" : u8"奈落 16x16");
    ImGui::BeginDisabled(m_piece.isSurface);
    int abyssLayer = ClampInt(m_piece.abyssLayer, 0, 7);
    if (ImGui::Combo(u8"奈落階層", &abyssLayer, kAbyssLayerLabels, IM_ARRAYSIZE(kAbyssLayerLabels)))
    {
        PushUndoSnapshot();
        m_piece.abyssLayer = abyssLayer;
        MarkPieceDirty();
    }

    int sublayerTag = static_cast<int>(m_piece.sublayerTag);
    if (ImGui::Combo(u8"上中下層タグ", &sublayerTag, kSublayerTagLabels, IM_ARRAYSIZE(kSublayerTagLabels)))
    {
        PushUndoSnapshot();
        m_piece.sublayerTag = static_cast<NarakuPiece::SublayerTag>(sublayerTag);
        MarkPieceDirty();
    }
    ImGui::EndDisabled();
...
void SceneNarakuPieceEditor::DrawPieceConnectionWindow()
{
    ...
    ImGui::SetNextWindowPos(ImVec2(16.0f, 480.0f), ImGuiCond_FirstUseEver);
    ...
    if (m_piece.isSurface)
    {
        ImGui::SeparatorText(u8"地上施設設定");
        ImGui::BeginChild("SurfaceFacilitySettings", ImVec2(0.0f, 240.0f), true, ImGuiWindowFlags_AlwaysVerticalScrollbar);

        static const char* kFacilityLabels[] = {
            u8"なし", u8"自宅", u8"商店", u8"武具屋", u8"レストラン＋受注受付", u8"奈落塔出入口" };
        static const char* kDirectionLabels[] = { u8"北", u8"南", u8"東", u8"西" };
        int facilityType = static_cast<int>(m_piece.surfaceFacility.type);
        if (ImGui::Combo(u8"地上施設", &facilityType, kFacilityLabels, IM_ARRAYSIZE(kFacilityLabels)))
        {
            PushUndoSnapshot();
            m_piece.surfaceFacility.type = static_cast<NarakuPiece::SurfaceFacilityType>(facilityType);
            MarkPieceDirty();
        }
        int facilityCell[2] = { m_piece.surfaceFacility.cell.x, m_piece.surfaceFacility.cell.z };
        if (ImGui::InputInt2(u8"施設左上セル", facilityCell))
        {
            PushUndoSnapshot();
            m_piece.surfaceFacility.cell = { facilityCell[0], facilityCell[1] };
            MarkPieceDirty();
        }
        if (ImGui::Button(u8"選択セルへ2x2施設を配置"))
        {
            PushUndoSnapshot();
            m_piece.surfaceFacility.cell = { m_selectedCellX, m_selectedCellZ };
            MarkPieceDirty();
        }
        int facing = static_cast<int>(m_piece.surfaceFacility.facing);
        if (ImGui::Combo(u8"施設正面", &facing, kDirectionLabels, IM_ARRAYSIZE(kDirectionLabels)))
        {
            PushUndoSnapshot();
            m_piece.surfaceFacility.facing = static_cast<NarakuPiece::Direction>(facing);
            MarkPieceDirty();
        }
        char modelPath[260] = {};
        std::snprintf(modelPath, sizeof(modelPath), "%s", m_piece.surfaceFacility.modelPath.c_str());
        if (ImGui::InputText(u8"施設モデル相対パス", modelPath, sizeof(modelPath)))
        {
        ...
        {
            if (ImGui::IsItemActivated()) PushUndoSnapshot();
            m_piece.surfaceFacility.offsetX = offset[0]; m_piece.surfaceFacility.offsetY = offset[1]; m_piece.surfaceFacility.offsetZ = offset[2];
            MarkPieceDirty();
        }
        float scale[3] = { m_piece.surfaceFacility.scaleX, m_piece.surfaceFacility.scaleY, m_piece.surfaceFacility.scaleZ };
        if (ImGui::DragFloat3(u8"施設拡大率", scale, 0.05f, 0.01f, 100.0f))
        {
            if (ImGui::IsItemActivated()) PushUndoSnapshot();
            m_piece.surfaceFacility.scaleX = scale[0]; m_piece.surfaceFacility.scaleY = scale[1]; m_piece.surfaceFacility.scaleZ = scale[2];
            MarkPieceDirty();
        }
        int rotation = m_piece.surfaceFacility.rotationQuarterTurns;
        if (ImGui::SliderInt(u8"施設Y回転(90度単位)", &rotation, 0, 3))
        {
            PushUndoSnapshot();
            m_piece.surfaceFacility.rotationQuarterTurns = rotation;
            MarkPieceDirty();
        }
    ...
    if (ImGui::Combo(u8"ステージ役割", &stageRoleIndex, kStageRoleLabels, IM_ARRAYSIZE(kStageRoleLabels)))
    {
        ...
        if (m_piece.stageRole != NarakuPiece::StageRole::Base)
        {
            m_piece.baseType = NarakuPiece::BaseType::None;
        }
        MarkPieceDirty();
    }

    int baseTypeIndex = static_cast<int>(m_piece.baseType);
    if (ImGui::Combo(u8"拠点種別", &baseTypeIndex, kBaseTypeLabels, IM_ARRAYSIZE(kBaseTypeLabels)))
    {
        PushUndoSnapshot();
        m_piece.baseType = static_cast<NarakuPiece::BaseType>(baseTypeIndex);
        m_piece.stageRole = m_piece.baseType == NarakuPiece::BaseType::None
            ? NarakuPiece::StageRole::Normal
            : NarakuPiece::StageRole::Base;
        MarkPieceDirty();
    }
...
void SceneNarakuPieceEditor::DrawGridObjectPlacementWindow()
{
    ...
    ImGui::TextUnformatted(u8"釣り地点は池・湖をクリックすると、隣接する歩行可能な陸地・溜りへ自動配置されます。");
    ...
    else if (m_gridObjectTool == GridObjectTool::Water)
    {
        static const char* kWaterDepthLabels[] = { u8"解除", u8"溜り", u8"池", u8"湖" };
        int waterDepth = static_cast<int>(m_newWaterDepth);
        if (ImGui::Combo(u8"水場属性", &waterDepth, kWaterDepthLabels, IM_ARRAYSIZE(kWaterDepthLabels)))
        {
            m_newWaterDepth = static_cast<NarakuPiece::WaterDepth>(waterDepth);
        }
    }
...
void SceneNarakuPieceEditor::DrawGridObjectSelectionWindow()
{
    ...
    case GridObjectKind::FishingPoint:
        if (m_selectedFishingPointIndex >= 0 &&
            m_selectedFishingPointIndex < static_cast<int>(m_piece.fishingPoints.size()))
        {
            NarakuPiece::FishingPointData& point = m_piece.fishingPoints[static_cast<size_t>(m_selectedFishingPointIndex)];
            ImGui::Text("%s (%d, %d)", u8"釣り地点（岸）", point.shoreCell.x, point.shoreCell.z);
            ImGui::Text("%s (%d, %d)", u8"対象水面", point.waterCell.x, point.waterCell.z);
            char idBuffer[128] = {};
            std::snprintf(idBuffer, sizeof(idBuffer), "%s", point.id.c_str());
            if (ImGui::InputText(u8"ID", idBuffer, sizeof(idBuffer)))
            {
                point.id = idBuffer;
                MarkPieceDirty();
            }
            if (ImGui::Button(u8"釣り地点を削除")) DeleteSelectedGridObject();
        }
        else
        {
            ClearGridObjectSelection();
            ImGui::TextUnformatted(u8"釣り地点が未選択です");
        }
        break;
...
void SceneNarakuPieceEditor::DrawNewPiecePopup()
{
    ...
    int pieceType = m_newPieceIsSurface ? 1 : 0;
    static const char* kNewPieceTypeLabels[] = { u8"奈落 16x16", u8"地上 8x8" };
    if (ImGui::Combo(u8"小ステージ種別", &pieceType, kNewPieceTypeLabels, IM_ARRAYSIZE(kNewPieceTypeLabels)))
    {
        m_newPieceIsSurface = pieceType == 1;
    }
    ...
    if (ImGui::Button(u8"作成", ImVec2(120.0f, 0.0f)))
    {
        ...
        else
        {
            CreateNewPiece(
                m_newPieceFileName,
                m_newPieceIsSurface ? NarakuPiece::SizePreset::Size8x8 : NarakuPiece::SizePreset::Size16x16);
...
void SceneNarakuPieceEditor::DrawSavePiecePopup()
{
    ...
    if (ImGui::Button(u8"保存", ImVec2(120.0f, 0.0f)))
    {
        ...
        else if (m_requestOpenUnreachableWalkablePopup)
        {
            ImGui::CloseCurrentPopup();
        }
    }
...
void SceneNarakuPieceEditor::DrawUnreachableWalkablePopup()
{
    EDITOR_PROFILE_FUNCTION();
    bool keepOpen = true;
    if (!ImGui::BeginPopupModal(
        u8"到達不能な歩行可能領域", &keepOpen, ImGuiWindowFlags_AlwaysAutoResize))
    {
        return;
    }
    EDITOR_PROFILE_WINDOW(u8"到達不能な歩行可能領域");

    ImGui::TextWrapped(
        u8"通常の進入地点から到達できない歩行可能領域が%d箇所、合計%dセルあります。",
        m_unreachableWalkableRegionCount,
        static_cast<int>(m_unreachableWalkableCells.size()));
    ImGui::TextWrapped(u8"保存前に対応方法を選択してください。対象セルは編集へ戻ると選択表示されます。");
    ImGui::Separator();

    ImGui::RadioButton(u8"ロープを設置して編集", &m_unreachableWalkableAction, 0);
    ImGui::RadioButton(u8"地形を調節", &m_unreachableWalkableAction, 1);
    ImGui::RadioButton(u8"そのまま保存", &m_unreachableWalkableAction, 2);

    if (ImGui::Button(u8"実行", ImVec2(120.0f, 0.0f)))
    {
        if (m_unreachableWalkableAction == 2)
        {
            if (SavePiece(m_pendingUnreachableSaveAsDraft, true))
            {
                m_unreachableWalkableCells.clear();
                m_unreachableWalkableRegionCount = 0;
                ImGui::CloseCurrentPopup();
            }
        }
        else
        {
            SelectUnreachableWalkableCells();
            if (m_unreachableWalkableAction == 0)
            {
                m_editMode = EditMode::GridObject;
                m_gridObjectTool = GridObjectTool::RopeTop;
                SetMessage(u8"到達不能セルを選択しました。ロープ上端と下端を配置してください");
            }
            else
            {
                m_editMode = EditMode::Height;
                SetMessage(u8"到達不能セルを選択しました。周囲との傾斜が35度以下になるよう調節してください");
            }
            ImGui::CloseCurrentPopup();
        }
    }
