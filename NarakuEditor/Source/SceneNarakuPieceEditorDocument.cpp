...
std::wstring SceneNarakuPieceEditor::GetCurrentSaveTargetPath() const
{
    ...
    if (m_piece.isSurface)
        return m_saveAsDraft ? NarakuPiece::MakeSurfaceDraftPiecePath(fileName)
            : NarakuPiece::MakeSurfaceCompletedPiecePath(fileName);
    return m_saveAsDraft ? NarakuPiece::MakeDraftPiecePath(m_piece.abyssLayer, fileName)
        : NarakuPiece::MakeCompletedPiecePath(m_piece.abyssLayer, fileName);
}

bool SceneNarakuPieceEditor::SavePiece(bool saveAsDraft, bool allowUnreachableWalkable)
{
    EDITOR_PROFILE_FUNCTION();
    ...
    if (!allowUnreachableWalkable)
    {
        const UnreachableWalkableResult unreachable = FindUnreachableWalkableRegions();
        if (!unreachable.cells.empty())
        {
            m_unreachableWalkableCells = unreachable.cells;
            m_unreachableWalkableRegionCount = unreachable.regionCount;
            m_unreachableWalkableAction = 1;
            m_pendingUnreachableSaveAsDraft = saveAsDraft;
            m_requestOpenUnreachableWalkablePopup = true;
            SetMessage(u8"通常経路から到達できない歩行可能領域があります。保存を保留しました");
            return false;
        }
    }
    ...
    std::string error;
    const std::wstring savePath = m_piece.isSurface
        ? (saveAsDraft ? NarakuPiece::MakeSurfaceDraftPiecePath(m_saveFileName)
            : NarakuPiece::MakeSurfaceCompletedPiecePath(m_saveFileName))
        : (saveAsDraft ? NarakuPiece::MakeDraftPiecePath(m_piece.abyssLayer, m_saveFileName)
...
bool SceneNarakuPieceEditor::LoadPieceFromPath(const std::wstring& path)
{
    ...
    const bool supportedAbyss = loadedPiece.sizePreset == NarakuPiece::SizePreset::Size16x16 &&
        loadedPiece.gridWidth == 16 && loadedPiece.gridDepth == 16;
    const bool supportedSurface = loadedPiece.isSurface &&
        loadedPiece.sizePreset == NarakuPiece::SizePreset::Size8x8 &&
        loadedPiece.gridWidth == 8 && loadedPiece.gridDepth == 8;
    if (!supportedAbyss && !supportedSurface)
    {
        SetMessage(u8"奈落16x16または地上8x8以外のピースは読込できません");
        return false;
    }
...
void SceneNarakuPieceEditor::CreateNewPiece(
    const std::wstring& fileName,
    NarakuPiece::SizePreset sizePreset)
{
    EDITOR_PROFILE_FUNCTION();
    ReleaseSurfaceFacilityPreviewModel();
    m_piece = NarakuPiece::CreateDefaultPiece(sizePreset);
    ...
    m_newWaterDepth = NarakuPiece::WaterDepth::Puddle;
...
void SceneNarakuPieceEditor::OpenLoadPieceDialog()
{
    ...
    const std::wstring initialPath = m_piece.isSurface
        ? NarakuPiece::MakeSurfaceDraftPiecePath(L"sample.json")
        : NarakuPiece::MakeDraftPiecePath(m_piece.abyssLayer, L"sample.json");
...
bool SceneNarakuPieceEditor::RenameCurrentPiece()
{
    ...
    const std::wstring newPath = m_piece.isSurface
        ? (m_saveAsDraft ? NarakuPiece::MakeSurfaceDraftPiecePath(newFileName)
            : NarakuPiece::MakeSurfaceCompletedPiecePath(newFileName))
        : (m_saveAsDraft ? NarakuPiece::MakeDraftPiecePath(m_piece.abyssLayer, newFileName)
...
bool SceneNarakuPieceEditor::DeleteCurrentPiece()
{
    ...
    const NarakuPiece::SizePreset currentSizePreset = m_piece.isSurface
        ? NarakuPiece::SizePreset::Size8x8
        : NarakuPiece::SizePreset::Size16x16;
    ...
    SavePieceHierarchyEntries();
    CreateNewPiece(displayName, currentSizePreset);
...
bool SceneNarakuPieceEditor::ReloadPieceHierarchyEntries()
{
    ...
    bool recoveredLastModified = false;
    const auto readPieceLastModified = [this](const std::wstring& relativePath)
    {
        const std::wstring piecePath = ResolvePieceHierarchyPath(relativePath);
        std::ifstream pieceStream(piecePath, std::ios::binary);
        std::string pieceLine;
        for (int lineIndex = 0; pieceStream && lineIndex < 32 && std::getline(pieceStream, pieceLine); ++lineIndex)
        {
            const std::size_t keyPosition = pieceLine.find("\"lastModified\"");
            if (keyPosition == std::string::npos)
            {
                continue;
            }
            const std::size_t colonPosition = pieceLine.find(':', keyPosition + 14);
            const std::size_t valueBegin = colonPosition == std::string::npos
                ? std::string::npos
                : pieceLine.find('"', colonPosition + 1);
            const std::size_t valueEnd = valueBegin == std::string::npos
                ? std::string::npos
                : pieceLine.find('"', valueBegin + 1);
            if (valueBegin != std::string::npos && valueEnd != std::string::npos)
            {
                return Utf8ToWide(pieceLine.substr(valueBegin + 1, valueEnd - valueBegin - 1));
            }
            break;
        }
        return std::wstring();
    ...
    while (std::getline(stream, line))
    {
        ...
        std::wstring lastModified = Utf8ToWide(lastModifiedUtf8);
        if (lastModified.empty())
        {
            lastModified = readPieceLastModified(relativePath);
            recoveredLastModified = recoveredLastModified || !lastModified.empty();
        }
        RegisterPieceHierarchyEntry(relativePath, isCompleted, lastModified);
...
void SceneNarakuPieceEditor::HandleMissingHierarchyEntry(const PieceHierarchyEntry& entry)
{
    ...
    const std::wstring normalizedEntryPath = ToLowerWide(NormalizePathSeparators(entry.relativePath));
    const bool isSurfaceEntry = normalizedEntryPath.find(L"/surface/") != std::wstring::npos;
    CreateNewPiece(
        m_saveFileName,
        isSurfaceEntry ? NarakuPiece::SizePreset::Size8x8 : NarakuPiece::SizePreset::Size16x16);
    m_saveAsDraft = !entry.isCompleted;
    SyncSaveFileNameInput();
    // 条件に該当する場合は、`m_piece.lastModified` の状態を更新します。
