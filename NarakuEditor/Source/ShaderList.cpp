...
float4 main(PS_IN pin) : SV_TARGET
{
	...
	float3 illumination = lightDiffuse.rgb * dotNL;
	float3 toPoint = pointPositionRange.xyz - pin.wPos.xyz;
	...
	illumination += pointColorEnabled.rgb * pointColorEnabled.w * pointAttenuation *
		saturate((dot(N, normalize(toPoint)) + 0.5f) / 1.5f);
	float3 fromSpot = pin.wPos.xyz - spotPositionRange.xyz;
	...
	illumination += spotColorEnabled.rgb * spotColorEnabled.w * spotCone * spotAttenuation *
		saturate((dot(N, normalize(-fromSpot)) + 0.5f) / 1.5f);
	illumination = saturate(illumination);
	float3 baseColor = color.rgb * objDiffuse.rgb;
	color.rgb = saturate(baseColor * max(illumination, 0.015f));
	color.rgb += objSpecular.rgb * illumination *
		pow(saturate(max(illumination.r, max(illumination.g, illumination.b))),
		max(0.01f, objSpecular.a) * 0.5f) * 0.5f;
	return color;
})EOT";
...
void ShaderList::SetExtendedLight(const ExtendedLight& light)
{
	ExtendedLight writable = light;
	m_pPS[PS_LAMBERT]->WriteBuffer(1, &writable);
}
