...
namespace NarakuMap
{
    ...
    float CalculateMaximumSlopeDegrees(float h00, float h10, float h01, float h11, float cellSize)
    {
        if (cellSize <= 0.0f)
        {
            return 90.0f;
        ...
        const float firstTriangleRise = std::sqrt(
            (h10 - h00) * (h10 - h00) +
            (h01 - h00) * (h01 - h00));
        const float secondTriangleRise = std::sqrt(
            (h11 - h01) * (h11 - h01) +
            (h11 - h10) * (h11 - h10));
        const float maximumRise = std::max(firstTriangleRise, secondTriangleRise);
        constexpr float kRadiansToDegrees = 57.29577951308232f;
        return std::atan(maximumRise / cellSize) * kRadiansToDegrees;
    }

    float GetCellMaximumSlopeDegrees(const TerrainLayer& layer, int cellX, int cellZ)
    {
        if (!IsCellIndexValid(layer, cellX, cellZ))
        {
            return 90.0f;
        ...
        return CalculateMaximumSlopeDegrees(
            GetVertexHeight(layer, cellX, cellZ),
            GetVertexHeight(layer, cellX + 1, cellZ),
            GetVertexHeight(layer, cellX, cellZ + 1),
            GetVertexHeight(layer, cellX + 1, cellZ + 1),
            layer.cellSize);
    }

    bool IsCellWalkable(const TerrainLayer& layer, int cellX, int cellZ)
    {
        if (!IsCellIndexValid(layer, cellX, cellZ))
        {
            return false;
        ...
        if ((flags & (CellAttributeBlocked | CellAttributeRemoved | CellAttributeWaterLake)) != 0u)
        {
            return false;
        }

        return GetCellMaximumSlopeDegrees(layer, cellX, cellZ) <= MaximumWalkableSlopeDegrees;
    }
    ...
    std::vector<ValidationIssue> ValidateMapData(const MapData& mapData)
    {
        ...
        for (std::size_t i = 0; i < mapData.bases.size(); ++i)
        {
            const BasePoint& base = mapData.bases[i];
            int layerIndex = -1;
            int cellX = -1;
            int cellZ = -1;
            if (!TryGetLayerAndCell(mapData, base.layerId, base.xz, layerIndex, cellX, cellZ))
                issues.push_back({ ValidationIssue::Error, u8"拠点中心が有効なセル範囲外にあります。" });
            if (base.footprintCells != 2 && base.footprintCells != 3)
                issues.push_back({ ValidationIssue::Error, u8"拠点の占有セル数が不正です。" });
            for (std::size_t j = i + 1; j < mapData.bases.size(); ++j)
                if (mapData.bases[j].type == base.type)
                    issues.push_back({ ValidationIssue::Error, u8"同じ種類の拠点が複数あります。" });
        }
    ...
    bool SaveMap(const wchar_t* path, const MapData& mapData, std::string* errorMessage)
    {
        ...
        AppendIndent(out, 1);
        out << "\"bases\": [\n";
        for (int i = 0; i < static_cast<int>(mapData.bases.size()); ++i)
        {
            const BasePoint& base = mapData.bases[i];
            AppendIndent(out, 2);
            out << "{ \"type\": \"" << (base.type == BaseType::SecondBase ? "second_base" : "forward_base") << "\""
                << ", \"x\": " << base.xz.x << ", \"z\": " << base.xz.z
                << ", \"layerId\": " << base.layerId
                << ", \"footprintCells\": " << base.footprintCells
                << ", \"modelOffsetX\": " << base.modelOffsetX
                << ", \"modelOffsetY\": " << base.modelOffsetY
                << ", \"modelOffsetZ\": " << base.modelOffsetZ << " }";
            out << (i + 1 < static_cast<int>(mapData.bases.size()) ? ",\n" : "\n");
        }
        AppendIndent(out, 1);
        out << "],\n";
        ...
        AppendIndent(out, 1);
        out << "\"fishingPoints\": [\n";
        for (int i = 0; i < static_cast<int>(mapData.fishingPoints.size()); ++i)
        {
            const FishingPoint& point = mapData.fishingPoints[i];
            AppendIndent(out, 2);
            out << "{ \"x\": " << point.xz.x << ", \"z\": " << point.xz.z
                << ", \"layerId\": " << point.layerId
                << ", \"lake\": " << (point.lake ? "true" : "false")
                << ", \"id\": \"" << EscapeJsonString(point.id) << "\" }";
            out << (i + 1 < static_cast<int>(mapData.fishingPoints.size()) ? ",\n" : "\n");
        }
        AppendIndent(out, 1);
        out << "]\n";
        out << "}\n";
    ...
    bool LoadMap(const wchar_t* path, MapData& outMapData, std::string* errorMessage)
    {
        ...
        const JsonValue* basesValue = nullptr;
        if (GetObjectValue<JsonValue>(rootValue, "bases", basesValue) && basesValue->type == JsonValue::TypeArray)
        {
            for (const JsonValue& baseValue : basesValue->arrayValue)
            {
                if (baseValue.type != JsonValue::TypeObject) continue;
                BasePoint base;
                std::string type;
                double number = 0.0;
                GetString(baseValue, "type", type);
                base.type = type == "forward_base" ? BaseType::ForwardBase : BaseType::SecondBase;
                GetNumber(baseValue, "x", number); base.xz.x = static_cast<float>(number);
                GetNumber(baseValue, "z", number); base.xz.z = static_cast<float>(number);
                GetNumber(baseValue, "layerId", number); base.layerId = static_cast<int>(number);
                if (GetNumber(baseValue, "footprintCells", number)) base.footprintCells = static_cast<int>(number);
                if (GetNumber(baseValue, "modelOffsetX", number)) base.modelOffsetX = static_cast<float>(number);
                if (GetNumber(baseValue, "modelOffsetY", number)) base.modelOffsetY = static_cast<float>(number);
                if (GetNumber(baseValue, "modelOffsetZ", number)) base.modelOffsetZ = static_cast<float>(number);
                loadedMap.bases.push_back(base);
            }
        }
        ...
        if (GetObjectValue<JsonValue>(rootValue, "fishingPoints", fishingPointsValue) && fishingPointsValue->type == JsonValue::TypeArray)
        {
            for (const JsonValue& pointValue : fishingPointsValue->arrayValue)
            {
                ...
                GetNumber(pointValue, "z", number); point.xz.z = static_cast<float>(number);
                GetNumber(pointValue, "layerId", number); point.layerId = static_cast<int>(number);
                if (GetBool(pointValue, "lake", boolValue)) point.lake = boolValue;
                GetString(pointValue, "id", point.id);
                loadedMap.fishingPoints.push_back(point);
            }
        }
        ...
        for (FishingPoint& point : loadedMap.fishingPoints)
        {
            if (FindLayerIndexById(loadedMap, point.layerId) < 0) point.layerId = fallbackLayerId;
        }
