...
namespace NarakuMap
{
    /** @brief プレイヤーと敵が歩行できる最大斜度（度）です。 */
    constexpr float MaximumWalkableSlopeDegrees = 35.0f;
    ...
    enum CellAttributeFlags : std::uint32_t
    {
        ...
        /** @brief 浅い溜りとして扱います。 */
        CellAttributeWaterPuddle = 1u << 6,

        /** @brief 歩行可能な池として扱います。 */
        CellAttributeWaterPond = 1u << 7,

        /** @brief 進入できない湖として扱います。 */
        CellAttributeWaterLake = 1u << 8,
    };
    ...
    /** @brief 生成マップ上の拠点中心と占有セル数です。 */
    struct BasePoint
    {
        BaseType type = BaseType::SecondBase;
        Vec2 xz;
        int layerId = 0;
        int footprintCells = 2;
    ...
    struct MapData
    {
        ...
        /** @brief 編集ピースの拠点属性から生成した拠点一覧です。 */
        std::vector<BasePoint> bases;
